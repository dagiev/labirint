...
class Player(GameSprite):
    ...
    def fire(self):
        bullet = Bullet('bullet.png', self.rect.centerx, self.rect.top, 40, 29, 15)
        bullets.add(bullet)
...
#back = (238, 130, 238)
back = transform.scale(image.load('siuu3.jpg'), (win_width, win_height))
...
w3 = GameSprite('platform1.png',16, 0, 300, 15)
w3 = GameSprite('platform1.png',16, 0, 300, 15)
...
while run:
    ...
    if finish == False: 
        ...
        if sprite.spritecollide(packman, monsters, True): #
            finish = True
            img = image.load('siuu3.png')
            window.fill((255, 255, 255))
            window.blit(transform.scale(img, (win_width, win_height)), (0, 0))
        
        if sprite.collide_rect(packman, final_sprite):
            finish = True
            img = image.load('siuu2.jpg')
            window.fill((255, 255, 255))
            window.blit(transform.scale(img, (win_width, win_height)), (0, 0))
